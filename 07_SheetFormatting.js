...
function createApplovinNewPivotTable(appData) { createUnifiedPivotTable(appData); }
...
function createUnifiedPivotTable(data) {
  ...
  if (!data || Object.keys(data).length === 0) {
    const headers = CURRENT_PROJECT === 'APPLOVIN_NEW' ? getApplovinNewHeaders() : getUnifiedHeaders();
  ...
  const wow = CURRENT_PROJECT === 'INCENT_TRAFFIC' ? 
    ...
    CURRENT_PROJECT === 'APPLOVIN_NEW' ?
    calculateApplovinNewWoWMetrics(data) :
  ...
  if (CURRENT_PROJECT === 'APPLOVIN_NEW') {
    applyApplovinNewFormatting(sheet, tableData.length, headers.length, formatData, data);
  } else {
...
function applyApplovinNewFormatting(sheet, numRows, numCols, formatData, appData) {
  const startTime = Date.now();
  ...
  try {
    ...
    const applovinNewColumnWidths = [
      { c: 1, w: 80 },   // Level
      { c: 2, w: 180 },  // Week Range / Date
      { c: 3, w: 40 },   // ID
      { c: 4, w: 40 },   // GEO
      { c: 5, w: 65 },   // Spend
      { c: 6, w: 55 },   // Spend WoW %
      ...
      { c: 16, w: 50 },  // D14/D7
      { c: 17, w: 50 },  // D30/D7
      { c: 18, w: 55 },  // eROAS 365d
      { c: 19, w: 120 }, // eProfit 730d
      { c: 20, w: 85 },  // eProfit WoW %
      { c: 21, w: 160 }, // Growth Status
      { c: 22, w: 450 }  // Comments
    ];
    
    applovinNewColumnWidths.forEach(col => {
    ...
    if (numRows > 1) {
      ...
      const eprofitRange = sheet.getRange(2, 19, numRows - 1, 1);
      eprofitRange.setHorizontalAlignment('right');
    }

    const rowTypeMap = { app: [], campaign: [], week: [], day: [] };
    formatData.forEach(item => {
      ...
      if (item.type === 'DAY') rowTypeMap.day.push(item.row);
    });
    ...
    if (rowTypeMap.day.length > 0) {
      const dayRanges = createOptimizedRanges(sheet, rowTypeMap.day, numCols);
    ...
    if (numRows > 1) {
      const numberFormatOperations = [
        ...
        { range: sheet.getRange(2, 19, numRows - 1, 1), format: '$0.0' }  // eProfit
      ];
      
      numberFormatOperations.forEach(op => op.range.setNumberFormat(op.format));
    }

    applyApplovinNewConditionalFormatting(sheet, numRows, appData);
    
    sheet.hideColumns(1);
    sheet.hideColumns(3);
    
    console.log(`APPLOVIN_NEW formatting completed in ${((Date.now() - startTime) / 1000).toFixed(1)}s`);
    
  } catch (e) {
    console.error('Error in applyApplovinNewFormatting:', e);
    throw e;
  }
}

function applyApplovinNewConditionalFormatting(sheet, numRows, appData) {
  try {
    const startTime = Date.now();
    ...
    // 2. Правила для ROAS колонок (D1, D3, D7, D14, D30)
    const roasColumns = [9, 10, 11, 12, 13]; // ROAS D1-D30
    roasColumns.forEach((col, index) => {
      const ruleIndex = conditionalFormatRequests.length;
          ...
          rule: {
            ...
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{
                  userEnteredValue: `=AND(NOT(ISBLANK(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()))), VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "%", "")) >= 100)`
                }]
              },
              format: {
                backgroundColor: hexToRgb('#d1f2eb'),
          ...
          rule: {
            ...
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{
                  userEnteredValue: `=AND(NOT(ISBLANK(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()))), VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "%", "")) >= 50, VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "%", "")) < 100)`
                }]
              },
              format: {
          ...
          rule: {
            ...
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{
                  userEnteredValue: `=AND(NOT(ISBLANK(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()))), VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "%", "")) < 50)`
                }]
              },
              format: {
                backgroundColor: hexToRgb('#f8d7da'),
    ...
    // 3. Правила для коэффициентов ROAS (D3/D1, D7/D3, D14/D7, D30/D7)
    const ratioColumns = [14, 15, 16, 17];
          ...
          rule: {
            ...
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{
                  userEnteredValue: `=AND(NOT(ISBLANK(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()))), VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "x", "")) >= 1.2)`
                }]
              },
              format: {
                backgroundColor: hexToRgb('#d1f2eb'),
          ...
          rule: {
            ...
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{
                  userEnteredValue: `=AND(NOT(ISBLANK(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()))), VALUE(SUBSTITUTE(INDIRECT("${String.fromCharCode(64 + col)}" & ROW()), "x", "")) < 0.8)`
                }]
              },
              format: {
                backgroundColor: hexToRgb('#f8d7da'),
    ...
    // 4. Правила для eProfit WoW колонки
    const eprofitWowColumn = 20;
    conditionalFormatRequests.push(
      {
        addConditionalFormatRule: {
          rule: {
            ranges: [{
              ...
              startRowIndex: 1,
              endRowIndex: numRows,
              startColumnIndex: eprofitWowColumn - 1,
              endColumnIndex: eprofitWowColumn
            }],
            booleanRule: {
              condition: {
                type: 'CUSTOM_FORMULA',
                values: [{ userEnteredValue: '=AND(ISNUMBER($T2), $T2>0)' }]
              },
              format: {
                backgroundColor: hexToRgb('#d1f2eb'),
                textFormat: { foregroundColor: hexToRgb('#0c5460') }
          ...
          index: conditionalFormatRequests.length
        }
      },
      {
        addConditionalFormatRule: {
    ...
    // 5. Правила для Growth Status колонки
    const growthColumn = 21;
    const statusColors = {
    ...
    console.log(`APPLOVIN_NEW conditional formatting completed in ${(endTime - startTime) / 1000}s (${conditionalFormatRequests.length} rules)`);
    
  } catch (e) {
    console.error('Error applying APPLOVIN_NEW conditional formatting:', e);
  }
}
