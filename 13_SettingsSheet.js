/**
 * Settings Sheet Management - Централизованные настройки в отдельном листе
 */
...
function createSettingsLayout(sheet) {
  ...
  sheet.getRange('A58').setValue(
    '• Auto Cache: TRUE = автосохранение комментариев в 2:00 каждый день\n' +
    '• Auto Update: TRUE = автообновление всех проектов по вторникам в 5:00\n' +
...
/**
 * Парсинг growth thresholds из строки
 */
function parseGrowthThresholds(healthyStr, inefficientStr, scalingStr) {
  let healthySpend = 10, healthyProfit = 5;
  if (healthyStr && healthyStr.includes('/')) {
    const parts = healthyStr.split('/');
    healthySpend = parseInt(parts[0].replace('%', '')) || 10;
    healthyProfit = parseInt(parts[1].replace('%', '')) || 5;
  }
  
  const inefficientProfit = parseInt(inefficientStr.replace('%', '')) || -8;
  const scalingSpend = parseInt(scalingStr.replace('%', '')) || -15;
  ...
  return {
    ...
    inefficientGrowth: { minSpendChange: 0, maxProfitChange: inefficientProfit },
...
function openSettingsSheet() {
  ...
  const spreadsheet = SpreadsheetApp.openById(MAIN_SHEET_ID);
  spreadsheet.setActiveSheet(sheet);
  SpreadsheetApp.getUi().alert('Settings Sheet', 'Лист Settings открыт. Вы можете редактировать настройки прямо в таблице.', SpreadsheetApp.getUi().ButtonSet.OK);
}
